<template>
  <div>
    ...
    <h2>基于业务场景拓展</h2>
    <p>
      当基础能力无法满足业务需求的时候，便需要基于业务场景拓展。这也是 LogicFlow
      能支持客服侧多个系统的关键所在。
    </p>
    <p>
    ...
    <p>
      API 拓展。支持在 LogicFlow 上注册自定义的方法，比如通过 API
      拓展提供图片下载的方法
    </p>
    <p>
      自定义节点、线。内置的矩形、圆形等图形类节点往往无法满足实际的业务需求，需要定义具有业务意义的节点。LogicFlow
      提供了
      的方式让用户定制具有自定义图形、业务数据的节点，比如流程审批场景中的
      “审批” 节点
    </p>
    <p>
      拓展组件。LogicFlow 在 SVG 图层上提供了 HTML
      层和一系列坐标转换逻辑，并支持在 HTML 层注册组件。宿主研发可以通过
      LogicFlow 的 API，基于任何 View
      框架开发组件，比如节点的右键菜单、控制面板等
    </p>
    <p>
      数据转换 adapter。LogicFlow
      默认导出的图数据不一定适合所有业务，此时可以通过 adapter API，在图数据从
      LogicFlow 输入、输出的时候做自定义转换，比如转换成 BPMN 规范的图数据
    </p>
    <p>
      内置部分拓展能力。基于上述拓展能力，我们还单独提供了 lf-extension
      的包，用来存放客服业务下沉淀出的具有通用性的节点、组件等，比如面向 BPMN
      规范的节点和数据 adapter，默认菜单。注意 lf-extension
      可以单独安装，并支持按需引入
    </p>
    <h3>
      绘制：绘制线的形状。 选中区域扩大：一般边设置的宽度较小( 1 -
      2px)，精确点击进行选中的实际操作比较困难，将选中区域扩大之后，可以使边更容易被鼠标选中。
      选中状态标识：将选中的边与其他边进行区分标识。
      位置调整：边的创建是通过锚点进行连接，自动计算其路径，但是路径会存在一些固定计算的弊端，可以通过手动调整位置，达到视觉最优。
      边文案设置：边上设置文案丰富信息表达。
      样式调整：系统提供了边的默认样式，例如：边的颜色为 #000000，宽度为
      2px，如果不同宿主系统的需要不同风格，LF 中也提供了样式调整的方法。
    </h3>
    ...
    <h3>数据转换 Adapter #LogicFlow 的数据格式</h3>
    <p>
      在 LogicFlow 中，一个流程图是由节点和边组成的。
      对于一个节点，我们需要知道这个节点的 id、类型、位置、文本、properties
      对于一个边，我们则需要知道这个边的
      id、类型、起始节点id（sourceNodeId）、目标节点id（targetNodeId）、文本、properties
      以及边的起点位置（startPoint），边的终点位置（endPoint）。
      折线的额外数据pointsList，因为折线是可以被用户手动调整的，所以增加此字段用于记录这个折线的具体路径。
      #类型 在 LogicFlow
      中，一个节点宽、高、颜色等表示外观的信息都不会保存到数据中，而是统一使用这个节点的类型来表示。例如我们通过
      LogicFlow
      的自定义机制定义一个节点为“开始节点(startNode)”，那么当前的这个项目中，就应该是知道这个
      type 为 startNode 的节点外观是什么样的。 #properties properties 是
      LogicFlow
      预留给开发者的一个空对象，开发者可以基于这个属性来绑定任何数据。上面的类型中提到，一个节点具体外观是通过类型来确定。但是当在项目中，需要基于某些业务条件，将这个节点外观进行一些调整时，我们可以将这些业务条件放到
      properties 中，然后在自定义节点的时候，通过this.props.model方法拿到
      properties，然后基于 proerties 中的内容重新设置这个节点的样式。
    </p>
    <h3>主题</h3>
    <p>
    ...
    <h3>监听器类别</h3>
    <p>按照监听的类型来划分，大致有三种： 执行监听器 任务监听器 事件监听器</p>
    <p></p>
  </div>
</template>